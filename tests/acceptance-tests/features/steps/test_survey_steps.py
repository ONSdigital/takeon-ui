...
def get_back_period_details(context, period):
...
def submit_test_survey_sales_values(context, validation_type, primary_value, comparison_value):
...
def switch_to_tab(context, tab_name):
...
def check_historic_data_tab(context):
...
def check_historic_data_with_current_period_data(context, values, tab_name):
